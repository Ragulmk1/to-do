...
const TodoList = () => {
    ...
    function create() {
        axios.post('http://localhost:5008/posting', { todo })
            .then(() => {
    ...
    function getData() {
        axios.get('http://localhost:5008/getting')
            .then((response) => {
    ...
    const updatedTodo = (id, updatedData) => {
        axios.put(`http://localhost:5008/updating/${id}`, { todo: updatedData })
            .then(() => {
    ...
    function deleteTodo(id) {
        axios.delete(`http://localhost:5008/deleting/${id}`)
            .then(() => {
                getData();
            })
            .catch(() => {
